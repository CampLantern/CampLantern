...
- **keywords:** value type, reference type, struct, class, boxing, string, StringBuilder, event, delegate, subscribe, unsubscribe, generic, async, await, CancellationToken, property, field, LINQ, foreach, fire-and-forget, async void, 무관찰 예외, UnobservedTaskException
...
- **keywords:** ClaudeBridge, bridge, inbox, outbox, unity_call, batch, headless, Editor 자동화, 씬 조작, GameObject 생성, 컴파일 확인, 임포트 경합, CS0246, rsp, scriptCompilationFailed, 병렬 에이전트, Play Mode 검증, 스모크 테스트, 플레이 검증, EnterPlaymode, ExitPlaymode, isPlaying, ScreenCapture, 스크린샷, 로그 기준점, Reflection.Invoke, Editor.log, 직접 파일 드롭, 봇 주입, 자가 검증 봇, 무인 통합테스트, Component.Add, stale 경고, CS0618 재출력, 리로드 경합, Type not found, Method not found, Editor.log 플러시, 봇 격리, 멀티 피어 Find, GetAllBehaviours
...
- **keywords:** Photon, Fusion, PUN, Voice, App Id, 넷코드, netcode, Meta Avatars, EOF, 백엔드, backend, 서버 DB, 패키지 버전, manifest.json, 음소거, mute, VoiceNetworkObject, Recorder, Speaker, 음성 아바타, AssembliesToWeave, 위빙, weave, NetworkProjectConfig, Unsupported Plugin, CleanBuildCache, 아바타, avatar, AvatarController, AvatarSpawnerFusion, FusionAvatarSdk28Plus, AvatarBehaviourFusion, FusionBBEvents, OvrAvatarManager, SampleInputManager, AvatarSdkManagerStyle2Meta, VRPlayerRig, OVRCameraRig, OVRManager, VR 리그, entitlement, Sample Scenes 임포트, AvatarEditorUtils, yRotate, Rebuild Prefab Table, 더미 러너, 유령 사본, 아바타 복제, RefreshAllActives, HideDummyRunnerProxies, TunnelingEffect, WallPenetrationTunneling, LocomotionTunneling, 컴포트 비네트, comfort vignette, 화면 암전, 벽 관통, PresetAvatars zip, assume-unchanged, LFS, SpawnDummyPersonAvatar, HasStateAuthority, HasInputAuthority, OnAvatarDataStreamChanged, RemoteAvatar, LocalAvatar, 1인칭 지오메트리, 더미 사람 표시, 플레이스홀더, DummyPlaceholder, 고정 위치 아바타, 텔레포트 착지면, 텔레포트 리티클, TeleportInteractable, ColliderSurface, ReticleDataTeleport, TeleportReticleDrawer, TeleportInteractor, TeleportControllerInteractor, TeleportMicrogestureInteractor, BodyTeleportInteractor, EnsureTeleportSurface, AddTeleportReticle, Mat_TeleportReticle, Unlit/Hotspot, Reticle-Circle, 아크 관통, 콜라이더 누락, ReplaceWithPrimitiveCollider, MeshCollider 금지, 나무 콜라이더, 바위 콜라이더, GetLocalBounds, 레이어마스크, layer mask, WireTeleportInteractable, TeleportCandidateComputer, AllowTeleport, DetectHit, InteractableRegistry, 텔레포트 장애물, 텔레포트 블로커, Obi, ObiCollider, OVROverlayCanvasSettings, OVROverlayCanvas, __pycache__, pyc, claude-bridge-mcp, 정체불명 파일, 저장소 정리
...
- **keywords:** 낚시, fishing, 캐스팅, 입질, 챔질, 파이팅, 릴링, 텐션, tension, 도망, 비늘털이, 스윙, swing QTE, 후킹, hooked, 줄 색, 미끼, 미끼 구매, 낚싯대, rod, 내구도, durability, 수리, AddBait, Repair, maxDurability, BaitPrice, RepairPrice, 물고기 FSM, Fish, FishState, FishingRod, FishingSpot 스포너, FishingFormulas, FishingTuning, FishingRodInput, FishingLoopSelfTest, 검증 봇, FishSpeciesData, fishId 매핑, 새 어종 추가, 실루엣, 에지 트리거, power, sizeMin, sizeMax, lengthMin, lengthMax, weightMin, weightMax, 공유 품질 롤, TODO(FORMULA), TODO(DATA), TODO(TUNING), RVRF, RvrfTackleFactory, Rod_Float_01, RealVRFishing, 낚싯대 비주얼, 비주얼 스왑, 그랩, grab, RodGrabber, 손잡이, 밑동, 피벗, 그립 오프셋
- **when to read:** 낚시(캐스팅/입질/파이팅/텐션·체력 수식/비늘털이 QTE) 코드 수정·어종 추가·튜닝 전 — domain/fishing-system.md 먼저, 수식 근거는 지시서 §7

### 전투 시스템 — [domain/combat-system.md](domain/combat-system.md) (구현 결정·암묵지, **구현 완료 2026-07-10**) + [gdd/전투_시스템_구현_지시서.md](gdd/전투_시스템_구현_지시서.md) (설계 스펙 — 인라인 규칙이 원본) + [gdd/전투_시스템_세부_기획서.docx](gdd/전투_시스템_세부_기획서.docx) (직접 Read 불가)
사냥터 전투 — 근접 유효타(스윕+오버랩), 몬스터 FSM(상태 패턴+오버레이), 어그로 6규칙, 스킬(hit window), 활/화살(자체 적분+경로 스윕), 플레이어 규칙(다운/소생/전멸/HP 스케일링/홀스터), Fusion 권한 분리(감지 클라·적용 권한자). 구현은 `Assets/Scripts/Combat/`(로컬, Fusion 무참조) + `Hunting/Networking`(네트워크 계층). 분해 계획·검증 절차는 `design/combat-detailed*/`(VERIFICATION.md 포함). 판정 규약·텔레포트 규약·봇 격리 조건·새 몬스터 추가 절차는 domain 문서 참조.
- **주의:** 구현 지시서 md는 범용 템플릿 문구가 섞여 있음 (프로젝트명 "Camp & Craft", 네임스페이스 `CampCraft.Combat`, 경로 `Docs/...`) — 치환표는 `design/combat-detailed/README.md`.
- **keywords:** 전투, combat, 근접, melee, 유효타, 무효타, 활, bow, 화살, arrow, 몬스터, monster, FSM, 어그로, aggro, 히스테리시스, 약점, weakpoint, 스킬, skill, SkillItem, 판정, hit window, hitWindowStart, hitWindowEnd, 판정 규약, 물리 콜백 금지, OnTriggerEnter 금지, IDamageable, ApplyDamage, HitInfo, HitVolume, 다운, 소생, 전멸, 홀스터, holster, 내구도, durability, HP 스케일링, 기여, contribution, MonsterData, MeleeWeaponData, BowData, ArrowData, CombatBalanceData, MeleeWeapon, MonsterController, MonsterHealth, AggroController, SkillRunner, SkillHitCheck, MonsterAnimationDriver, PlayerHealth, PlayerReviver, WeaponHolster, Blacksmith, CombatPlayers, CombatScenes, NotifyTeleported, 텔레포트, 유령 타격, NetworkedHuntMonster, NetworkedCombatPlayer, NetworkedPlayerBodies, HuntMonster_Bear, CombatSandbox, 검증 봇, SelfTest, NetHuntTest, 권한 분리, State Authority, 새 몬스터 추가, RewardGranted, 사냥 보상 이벤트, 이중 계보, 이동 방식, 로코모션, locomotion, 텔레포트 이동, MovingSetting, ControllerMovement, Slide, 고릴라 태그, 회피 밸런싱, telegraph, 예고 시간
- **when to read:** 전투 코드 수정·몬스터/무기 추가·튜닝·네트워크 전투 확장 전 — domain/combat-system.md 먼저, 규칙 원문은 지시서
