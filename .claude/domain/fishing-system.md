...
- `FishingRod` — 얇은 컨트롤러: `Cast(Fish)`/`Chamjil()`/`SetReeling(bool)`/미끼·내구도(RodData) + 보충 API `AddBait(int)`/`Repair()`(2026-07-13). 획득은 `FishCaught(FishInstance, FishReward)` 이벤트로만 알림 — **Wallet/Inventory 적용과 미끼/수리 결제는 하네스 책임** (가격은 §9 `BaitPrice`/`RepairPrice` 경유만).
...
- **미끼/수리 코인 싱크는 하네스에서 결제한다** (2026-07-13): 하네스가 `Wallet.TrySpend(FishingFormulas.BaitPrice()/RepairPrice())` 성공 시에만 `rod.AddBait()`/`Repair()` 호출 + 즉시 저장. 결제 로직을 FishingRod에 넣지 말 것 (얇은 컨트롤러 원칙). `RodData.maxDurability`는 필드 추가 전 직렬화된 씬에서 0으로 로드되므로 `FishingRod.Awake`가 보정한다 — 씬 재생성 불필요.
