...
- **로비 VR 허브 구현 (2026-07-15)**: 로비는 이제 실제 VR 플레이 가능한 포탈 허브다 — `LobbyMapFactory`(`Tools > Make Assets > Build Lobby Map (VR)`, idempotent)가 캠프장 환경(플레이스홀더 아트)·석양 라이팅·VR 포탈 UI를 구성하고 `RoomScenesFactory.CreateLobby`가 씬 생성 시 공통 호출한다. VR 이동은 `UI/PortalPanelController`(월드스페이스 패널 버튼 → `SceneManager.LoadScene`)가 담당하며 IMGUI `LobbyHarness`는 데스크톱 개발용으로만 남는다. 스폰포인트 GO 이름은 **"PlayerSpawn"**(컴포넌트는 `PlayerSpawnPoint`) — 팩토리는 컴포넌트 존재 기준으로 중복 생성을 건너뛰므로 이름에 의존하지 말 것.
