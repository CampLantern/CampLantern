...
- **로비 VR 허브 구현 (2026-07-15)**: 로비는 이제 실제 VR 플레이 가능한 포탈 허브다 — `LobbyMapFactory`(`Tools > Make Assets > Build Lobby Map (VR)`, idempotent)가 캠프장 환경(플레이스홀더 아트)·석양 라이팅·VR 포탈 UI를 구성하고 `RoomScenesFactory.CreateLobby`가 씬 생성 시 공통 호출한다. VR 이동은 `UI/PortalPanelController`(월드스페이스 패널 버튼 → `SceneManager.LoadScene`)가 담당하며 IMGUI `LobbyHarness`는 데스크톱 개발용으로만 남는다. 포탈은 **GDD 그림 1 "공간 구조 흐름도"대로 목적지별 관문 3개가 세 갈래 길로 분기**하는 구조다(단일 안내판 아님) — 낚시터=파랑/사냥터=주황(존 페넌트 3개)/영지=초록 색 코딩, 관문마다 단일 버튼 패널.
- **활동 공간 VR 맵 (2026-07-15)**: 낚시터·사냥터·영지도 `RoomMapsFactory`(`Tools > Make Assets > Build All Room Maps (VR)`, 씬별 메뉴도 있음)가 맵을 구성하고 `RoomScenesFactory.CreateXxx`가 씬 생성 시 공통 호출한다. 공용 조립 헬퍼는 `MapBuildUtil`(프리미티브 파트·관문·라이팅 프리셋). 테마: 낚시터=호숫가 캠프(부두·갈대·모래톱, 맑은 아침), 사냥터=존A 숲(그림 2 — **사냥감 스폰 공터 x±8/z 0~15는 비워 둠**, 늦은 오후), 영지=**그림 3 레이아웃 목업 그대로**(울타리 필지+남쪽 입구, 중앙 캠프파이어 소셜존, 쉘터존 좌상/트로피룸 우상/방명록 좌하/전시 슬롯 우하, 동쪽 확장 예정 구역, 초저녁). 세 씬 공통으로 **로비 귀환 관문**(회색, "귀환" 버튼)과 스폰·EventSystem이 보장된다. 게임플레이 오브젝트(Pond/Stations/CookingPot/사냥감 스폰)는 맵 재빌드에도 건드리지 않는다(idempotent — 환경/관문 루트만 재구성). 스폰포인트 GO 이름은 **"PlayerSpawn"**(컴포넌트는 `PlayerSpawnPoint`) — 팩토리는 컴포넌트 존재 기준으로 중복 생성을 건너뛰므로 이름에 의존하지 말 것.
