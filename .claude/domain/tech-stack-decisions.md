...
- **아크 관통 — 콜라이더 자체 문제는 아니었다(2026-07-20 확인·부분 수정)**: 텔레포트 아크가 나무·바위 등 환경 소품을 뚫고 지나가 뒤쪽 바닥을 착지 지점으로 표시하는 문제. 처음엔 "소품에 콜라이더가 없어서"로 추정했으나 실측 결과 **FantasyEnvironments 실물 나무 프리팹 19종 전부 자체 `CapsuleCollider`를 이미 갖고 있었다**(콜라이더 자체는 있음). 바위는 프리팹마다 달라 3종 중 2종은 콜라이더 없음, 1종은 자체 `MeshCollider` 보유.
  ...
  - **여전히 남은 진짜 원인(미수정 — 사용자가 "레이어 문제 같으니 보기만 하라"고 명시)**: 나무는 콜라이더가 이미 있었는데도 아크가 뚫고 지나간다는 게 재확인되면, 콜라이더 유무가 아니라 **`TeleportInteractor`(또는 그 아크 시뮬레이션)의 장애물 감지 레이어마스크가 나무/바위 소품의 레이어를 제외하고 있을 가능성**이 높다 — 사용자 본인이 이렇게 진단했고 이번 세션에서는 손대지 않았다. 다음에 고치려면 `BodyTeleportInteractor`(및 좌우 `TeleportControllerInteractor`) 컴포넌트의 레이어마스크 필드와, 나무·바위 GameObject가 실제로 어느 레이어에 있는지(대개 Default겠지만 `PlaceEnv`가 레이어를 지정하지 않으므로 소스 프리팹의 레이어를 그대로 물려받는다) 비교해볼 것.
