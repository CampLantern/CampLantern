...
- **프리팹 생성** — `VRPlayerRigFactory`(VR 리그), `StationFactory`(낚시/요리 스테이션), `VRUIFactory`(VR UI 세트: 패널·버튼·EventSystem, `Import TMP Essentials`·`Add Interaction To VR Rig` 메뉴 포함), `CookingPhysicalFactory`(**Physical Cooking** — 물리 요리용 재료 프록시·국자 그랩 프리팹, Resources에 생성해 런타임 스폰). 전부 `Tools > Make Assets` 아래.
- **맵 실사화 파이프라인(2026-07-16)** — 환경 에셋은 별도 보관 프로젝트(`D:\UnityProjects\Assets`)에서 필요한 파일+.meta만 복사해 온다. `EnvMaterialUrpConverter`(**Convert Env Materials To URP** — FantasyEnvironments가 빌트인 셰이더라 임포트 후 필수), `EnvBuildRunner`(**Capture Map Screenshots** — 씬 프리뷰 카메라 렌더를 `Library/MapShots`에 저장, 배치모드 진입점 `RunAll`), `MapAuditRunner`(**Audit Map Placement** — 공중부양·구역 침범·스폰 압박 수치 감사), `EnvAssetPruner`(**Prune Unused Env Assets** — 씬 의존성+코드 참조 기준 미사용 삭제. **맵 팩토리의 프리팹 이름 세트 변경 시 `k_codeReferencedPrefabs` 동기화 필수**). 맵 빌드 헬퍼(`MapBuildUtil.PlaceEnv` 등)는 팩 미임포트 시 프리미티브 폴백으로 동작한다.
- **P0 플레이 자동 검증(2026-07-22)** — `P0AutoVerifyRunner`(**Tools > P0 Play Test > Run Auto Verification**, 배치 진입점 `RunAll`)가 저장 파일을 `.bak` 백업 후 신규 상태로 Play에 진입하고, `P0AutoVerifyDriver`(Bootstrap, SessionState 플래그로만 활성)가 낚시 FSM 완주→요리·판매→구매·배치→영속성 라운드트립→2피어 협동 사냥(재사냥 포함)을 실제 API로 구동해 `Library/P0VerifyReport.json`에 기준별 PASS/FAIL 기록. 자동화 불가 항목(VR 젓기·UI 레이 클릭·음성 실효·페이싱)은 SKIP으로 명시 — 실기 수동 확인 대상.
...
- **요리는 물리 인터랙션(2026-07-16)**: UI "투입/조리" 버튼 대신 재료 프록시(`IngredientPickup` — 선반 `IngredientShelf`가 인벤토리 종류별 1개씩 꺼내 놓음)를 손으로 집어 냄비 림 트리거(`PotInteractionZone`)에 넣고, 국자(`StirTool`)로 저어 `Cook()` 발동(젓기 누적 거리 + 햅틱). **프록시는 스폰 시 인벤토리를 차감하지 않고 냄비 투입 시점에만 `TryAddIngredient`로 차감** — 인벤토리가 항상 권위라 프록시 복제·유실이 데이터에 영향 없음(재고 없으면 튕겨냄). 조립은 `PhysicalCookingBuilder`가 하네스 Start에서 코드 생성(씬/프리팹 수정 없음), 그랩 프리팹 2종만 `CookingPhysicalFactory` 산출물(Resources) 사용. 젓기·제스처 임계값은 `TODO(TUNING)` — 실기 확정 대상.
